...
class CLlist extends CLComponent {
  ...
  handleReset = () => {
    SessionManagement.destroySession(this.props.settings.sessionCode);
    this.props.form.resetFields();
    this.props.settings.handleReset && this.props.settings.handleReset();
  }
    ...
    for (let i = 0; i < operation.length; i++) {
      switch (operation[i].type) {
        case 'text':
          formItem = (
            <FormItem {...formItemLayout} label={`${operation[i].label}`} key={`${operation[i].id}${i}`}>
              {getFieldDecorator(`${operation[i].id}`)(<Input maxLength={`${operation[i].wordLength}`} placeholder={`${operation[i].placeholder}`} />)}
            </FormItem>
          );
          break;
        case 'number':
          ...
          formItem = (
            ...
            >
              {getFieldDecorator(`${operation[i].id}`)(
                <Select mode={operation[i].mode} placeholder={`${operation[i].placeholder}`} onSelect={that.props.settings.onSelect} allowClear>
                  {
        ...
        case 'monthPicker':
          formItem = (
            <FormItem {...formItemLayout} label={`${operation[i].label}`} key={`${operation[i].id}${i}`}>
              {getFieldDecorator(`${operation[i].id}`)(<DatePicker.MonthPicker
                showTime
                size="small"
                style={{ maxWidth: '100%', fontSize: 10 }}
                allowClear={false}
  ...
  render() {
    const {
      columns, data, operation, defaultdate, defaultbtn, defaultbtnTow, defaultbtnLeft,
      pagination, pageChange, tableLoading,
      btn, btnTow, rowClass, rowSelection, expandedRowRender, expandedRowKeys, scroll = {}, columnGroup,
    } = this.props.settings;
    ...
    return (
      <div className="cllist" key="cllist">
          ...
          >
            ...
            <Row>
              <Col span={24} style={{ textAlign: 'right' }}>
                {btnTow && btn.length ? btnTow.map((doc) => {
                  return (<Button key={doc.title} type={doc.type} style={{ marginRight: 8 }} onClick={doc.fn}>{doc.title}</Button>);
                }) : ''}
        ...
        {defaultdate && defaultdate.length ? defaultdate.map((doc) => {
          return (<p style={{ textAlign: 'right', marginRight: 35 }} key={doc.name}>{doc.name}</p>);
        }) : ''}
        <Row>
          <Col span={2}>
          {defaultbtnLeft && defaultbtnLeft.length ? defaultbtnLeft.map((doc) => {
              return (<Button key={doc.title} type={doc.type} onClick={doc.fn} style={{ marginLeft: 8 }}>{doc.title}</Button>);
            }) : ''}
          </Col>
          <Col span={22} style={{ textAlign: 'right' }}>
            {defaultbtn && defaultbtn.length ? defaultbtn.map((doc) => {
              return (<Button key={doc.title} type={doc.type} onClick={doc.fn} style={{ marginRight: 8 }}>{doc.title}</Button>);
            }) : ''}
          </Col>
          <Col span={2} style={{ textAlign: 'right' }}>
            {defaultbtnTow && defaultbtnTow.length ? defaultbtnTow.map((doc) => {
              return (<Button key={doc.title} type={doc.type} onClick={doc.fn} style={{ marginLeft: 8 }}>{doc.title}</Button>);
            }) : ''}
          </Col>
        </Row>
  ...
  mapPropsToFields(props) {
    ...
    _.each(search, (value, index) => {
      if (['beginTime', 'startDate', 'beginCreateTime'].indexOf(index) > -1) {
        ...
        formSearch[index] = Form.createFormField({ value: moment(new Date(value)) });
      } else if (_.indexOf(['filingDateList'], index) > -1) {
        formSearch[index] = Form.createFormField({ value: moment(new Date(value[0])) });
      } else {
