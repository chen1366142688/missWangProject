...
var myreg = /^\d{10}$/;
var myregs = /^\d{4}$/;
var domain = "/uplending-external-web";
...
function verifyCode() {
    phone = $('.phoneNum').val();
    $.ajax({
        url:`${domain}/member/verifyCode?phone=${"63" + phone}&version=0`,
        type:'GET',
        success:function(res){
...
function validExist(phone, value) {
    if(phone.length == 10){
        ...
        phone = "63" + phone;
        $.ajax({
            url: `${domain}/member/${phone}/exist?version=0`,
            type:'GET',
            dataType:"json",
            success:function(res){
...
$('.down').on('click',function(){
    ...
    location.href = 'http://www.cashlending.ph/XCash.apk';
})
